...
// 文件名称：SerialOutboundForm.xaml.cs
// 创建时间：2025-01-20
...
/// 序列号出库表单窗口（扫描出库）
/// </summary>
public partial class SerialOutboundForm : Window
{
    private readonly ILocalizationManager? _localizationManager;
    private SerialOutboundFormViewModel? _viewModel;

    /// <summary>
    /// 初始化序列号出库表单窗口
    /// </summary>
    /// <param name="vm">序列号出库表单视图模型</param>
    /// <param name="localizationManager">本地化管理器</param>
    public SerialOutboundForm(SerialOutboundFormViewModel vm, ILocalizationManager? localizationManager = null)
    {
        InitializeComponent();
        ...
        // 窗口加载时重新加载选项数据，确保每次打开都刷新
        Loaded += async (s, e) =>
        {
            if (_viewModel != null)
            {
                // 清空现有数据
                _viewModel.DestCodeOptions.Clear();
                _viewModel.DestPortOptions.Clear();
                
                // 重新加载数据
                await _viewModel.InitializeDestCodeOptionsAsync();
                await _viewModel.InitializeDestPortOptionsAsync();
            }
            
            // 刷新窗口背景以适配 Material Design 主题
            if (TryFindResource("MaterialDesignPaper") is System.Windows.Media.Brush backgroundBrush)
            {
        ...
        Loaded += (s, e) =>
        {
            ...
            _ = Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                if (_viewModel != null)
                {
                    ...
                    {
                        if (args.PropertyName == nameof(SerialOutboundFormViewModel.FullSerialNumber))
                        {
    ...
    private void CalculateAndSetOptimalHeight()
    {
        ...
        // 计算字段数量
        const int fieldCount = 4; // 出库单号、出库日期、目的地港口、完整序列号

        // 每个字段 StackPanel：MinHeight=56（普通字段）或 80（完整序列号）
        const double normalFieldHeight = 56;
        const double fullSerialNumberFieldHeight = 80;
        ...
        const double normalFieldCount = fieldCount - 1; // 除了完整序列号字段

        double fieldsHeight = (normalFieldCount * normalFieldHeight) + fullSerialNumberFieldHeight;
    ...
    private async Task CheckDuplicateAsync(string fullSerialNumber)
    {
        ...
        {
            // 检查重复失败不影响输入，只记录日志
            System.Diagnostics.Debug.WriteLine($"[SerialOutboundForm] 检查序列号重复异常：{ex.Message}");
        }
    }
    ...
    private T? FindVisualChild<T>(DependencyObject parent, string? name = null) where T : DependencyObject
    {
        for (int i = 0; i < System.Windows.Media.VisualTreeHelper.GetChildrenCount(parent); i++)
        {
            ...
            if (child is T t)
            {
                ...
                // 如果指定了名称，检查 FrameworkElement 的 Name
